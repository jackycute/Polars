...
class Level101 {
    ...
    previouslevel = emo.Sprite("previouslevel.png");
    replay = emo.Sprite("replay.png");
    
    // create map sprite with 32x32, 2 pixel border and 2 pixel margin
    background = TMXMapSprite("level101.tmx", 0, "background.png", MAP_BLOCK_SIZE, 0, 0);
    cloud = TMXMapSprite("level101.tmx", 1, "cloud_chip.png", MAP_BLOCK_SIZE, 0, 0);
    mountain = TMXMapSprite("level101.tmx", 2, "mountain_chip.png", MAP_BLOCK_SIZE, 0, 0);
    ice_back = TMXMapSprite("level101.tmx", 3, "ice_chip.png", MAP_BLOCK_SIZE, 0, 0);
    object = TMXMapSprite("level101.tmx", 4, "ice_chip.png", MAP_BLOCK_SIZE, 0, 0);
    ice_front = TMXMapSprite("level101.tmx", 5, "ice_chip.png", MAP_BLOCK_SIZE, 0, 0);
    interact = TMXMapSprite("level101.tmx", 6, "interact_chip.png", MAP_BLOCK_SIZE, 0, 0);
    ...
    function onLoad() {
        ...
        wallN.setSize(background.getWidth(), 20);
        wallS.setSize(background.getWidth(), 20);
        wallE.setSize(20, background.getHeight());
        wallW.setSize(20, background.getHeight());
        ...
        previouslevel.move(replay.getX() - previouslevel.getWidth(), 0);
        ...
        wallN.move(0, 0);
        wallS.move(0, background.getHeight() - wallS.getHeight());
        wallE.move(0, 0);
        wallW.move(background.getWidth()  - wallW.getWidth(), 0);
    ...
    function CameraMoveByStage(sprite) {
        ...
        if(stageZoomWidth >= background.getWidth()) BIAS_X = -(stageZoomWidth - background.getWidth()) / 2;
        else if(BIAS_X <= 0) BIAS_X = 0;
        else if(BIAS_X + stageZoomWidth >= background.getWidth()) BIAS_X = background.getWidth() - stageZoomWidth;
        
        if(stageZoomHeight >= background.getWidth()) BIAS_Y = -(stageZoomHeight - background.getHeight()) / 2;
        else if(BIAS_Y <= 0) BIAS_Y = 0;
        else if(BIAS_Y + stageZoomHeight >= background.getHeight()) BIAS_Y = background.getHeight() - stageZoomHeight;
    ...
    function onMotionEvent(mevent) {
        ...
        if (mevent.getAction() == MOTION_EVENT_ACTION_DOWN) {
        	...
        	if(previouslevel.contains(mevent.getX(), mevent.getY())) {
        		Current_Level--;
				LoadScript(Current_Level);
        	}
...
Current_Level_Function = Level101();