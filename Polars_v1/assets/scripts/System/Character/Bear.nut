...
BEAR_PHYSICS_STILL_THRESHOLD 	<- 0.01;
...
enum BEAR_LIFE_STATUS {
	NORMAL, DAMAGED, SUPER, DIE, DROWN
}
...
class Bear extends emo.SpriteSheet {
	...
	function DrownBear() {
		bear_life_status = BEAR_LIFE_STATUS.DROWN;
			
		bear.hide();
			
		local bear_drown = emo.Sprite("Bear_Drown.png");
		bear_drown.move(bear.getX(), bear.getY());
		bear_drown.setZ(bear.getZ());
		bear_drown.load();
			
		if (bear_drown.getPhysicsInfo() != null) {
		   	bear_drown.getPhysicsInfo().remove();
			bear_drown.physicsInfo = null;
		}
	...
	function onGameLoop() {
		...
		if(this.getY() + this.getHeight() > stage.getWindowHeight() - MAP_BLOCK_SIZE) {
			this.DrownBear();
		} else {
			UpdateStatus();
			UpdateAnimate();
