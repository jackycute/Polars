...
BOUNCE_FIXROTATION		<- true;
BOUNCE_LINEARDAMPING	<- 0;
BOUNCE_ALLOWSLEEP		<- true;
...
INTERACT_TILE <- {
	...
	[62] = {
		name = "Single Bounce Cube"
		sprite_create_method = "rectangle",
		rectangle_size = [MAP_BLOCK_SIZE, MAP_BLOCK_SIZE],
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		fixturedef_friction = BOUNCE_FRICTION,
		fixturedef_restitution = BOUNCE_RESTITUTION,
		bodydef_fixrotation = BOUNCE_FIXROTATION,
		bodydef_lineardamping = BOUNCE_LINEARDAMPING,
		bodydef_allowsleep = BOUNCE_ALLOWSLEEP,
		function onCreate(map, tile_coord) {
		}
		function onUpTime(this_sprite) {
		},
	...
	[10] = {
		name = "Continuous 32-Width Vertical Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		...
		function constructor(map, tile_coord) {
			local tile_height = 1;
			for(local j = tile_coord.y + 1;;j++) {
				if(map.getTileAt(j, tile_coord.x) == 26)
					tile_height++;
				else if(map.getTileAt(j, tile_coord.x) == 42) {
					tile_height++;
					local temp = emo.Rectangle();
					temp.setSize(MAP_BLOCK_SIZE, tile_height*MAP_BLOCK_SIZE);
					//print("Meet Ground End");
	...
	[11] = {
		name = "Continuous 16-Width Vertical Align-Left Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		...
		function constructor(map, tile_coord) {
			local tile_height = 1;
			for(local j = tile_coord.y + 1;;j++) {
				if(map.getTileAt(j, tile_coord.x) == 27)
					tile_height++;
				else if(map.getTileAt(j, tile_coord.x) == 43) {
					tile_height++;
					local temp = emo.Rectangle();
					temp.setSize(MAP_BLOCK_SIZE/2, tile_height*MAP_BLOCK_SIZE);
					//print("Meet Ground End");
	...
	[12] = {
		name = "Continuous 16-Height Vertical Align-Right Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		...
		function constructor(map, tile_coord) {
			local tile_height = 1;
			for(local j = tile_coord.y + 1;;j++) {
				if(map.getTileAt(j, tile_coord.x) == 28)
					tile_height++;
				else if(map.getTileAt(j, tile_coord.x) == 44) {
					tile_height++;
					local temp = emo.Rectangle();
					temp.setSize(MAP_BLOCK_SIZE/2, tile_height*MAP_BLOCK_SIZE);
					//print("Meet Ground End");
	...
	[13] = {
		name = "Continuous 32-Height Horizontal Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		...
		function constructor(map, tile_coord) {
			local tile_width = 1;
			for(local i = tile_coord.x + 1;;i++) {
				if(map.getTileAt(tile_coord.y, i) == 14)
					tile_width++;
				else if(map.getTileAt(tile_coord.y, i) == 15) {
					tile_width++;
					local temp = emo.Rectangle();
					temp.setSize(tile_width*MAP_BLOCK_SIZE, MAP_BLOCK_SIZE);
					//print("Meet Ground End");
		...
		function onImpact(fixtureA, fixtureB, position, normal, normalImpulse, tangentImpulse) {
			if(fixtureA.id == bear.getFixture().id || fixtureB.id == bear.getFixture().id)
				if(normal.y == -1) bear.DrownBear();
		},
		function onContact(state, fixtureA, fixtureB, position, normal, normalImpulse, tangentImpulse) {
		}
	},
	[29] = {
		name = "Continuous 16-Height Align-Top Horizontal Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		...
		function constructor(map, tile_coord) {
			local tile_width = 1;
			for(local i = tile_coord.x + 1;;i++) {
				if(map.getTileAt(tile_coord.y, i) == 30)
					tile_width++;
				else if(map.getTileAt(tile_coord.y, i) == 31) {
					tile_width++;
					local temp = emo.Rectangle();
					temp.setSize(tile_width*MAP_BLOCK_SIZE, MAP_BLOCK_SIZE/2);
					//print("Meet Ground End");
	...
	[45] = {
		name = "Continuous 16-Height Align-Bottom Horizontal Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
		...
		function constructor(map, tile_coord) {
			local tile_width = 1;
			for(local i = tile_coord.x + 1;;i++) {
				if(map.getTileAt(tile_coord.y, i) == 46)
					tile_width++;
				else if(map.getTileAt(tile_coord.y, i) == 47) {
					tile_width++;
					local temp = emo.Rectangle();
					temp.setSize(tile_width*MAP_BLOCK_SIZE, MAP_BLOCK_SIZE/2);
					//print("Meet Ground End");
	...
	[58] = {
		name = "Single 16-Width Align-Left Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[59] = {
		name = "Single 16-Width Align-Right Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[60] = {
		name = "Single 16-Height Align-Top Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[61] = {
		name = "Single 16-Height Align-Bottom Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[74] = {
		name = "Single 16X16 Top-Left Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[75] = {
		name = "Single 16X16 Top-Right Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[76] = {
		name = "Single 16X16 Bottom-Left Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
	...
	[77] = {
		name = "Single 16X16 Bottom-Right Bounce Cube"
		sprite_create_method = "self_construct",
		using_physics = true,
		physics_create_method = "static",
		fixturedef_density = BOUNCE_DENSITY,
