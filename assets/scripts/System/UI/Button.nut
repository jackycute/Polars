class LONGPRESS_TIMER extends Timer {
	...
	function onFinish() {
		//print("onFinish");
		if((master.getObject() != null && master.getObject().contains(mevent.getX(), mevent.getY()))
	...
	function onMotionEvent(mevent) {
		...
		switch(mevent.getAction()) {
			case MOTION_EVENT_ACTION_DOWN:
				if(touch_id == null)
				if((object != null && object.contains(mevent.getX(), mevent.getY())) 
					|| (text != null && text.contains(mevent.getX(), mevent.getY()))) {
					touch_id = id;
					if(down_time == 0) {
